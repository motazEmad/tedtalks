...
public interface TedTalkRepository extends CrudRepository<TedTalk, Long> {
    ...
    @Query("SELECT new com.io.tedtalks.entity.TopSpeaker(t.author, SUM(t.views), SUM(t.likes)) FROM TedTalk t GROUP BY t.author ORDER BY SUM(t.views) DESC, SUM(t.likes) DESC LIMIT :limit")
